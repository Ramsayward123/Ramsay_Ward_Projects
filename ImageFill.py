...
class ImageGraph:
    ...
    def print_adjacency_matrix(self):
        ...
        for node_index in range(0, len(self.nodes)):
            for column in range(0, len(self.nodes)):
                if column in self.nodes[node_index].edges:
                    print('1', end='')
                else:
                    print('0', end='')
            print()
    ...
    def bfs(self, start_index, color):
        ...
        while not frontier_queue.is_empty():
            ...
            current_vertex = self.find_node(current_index)
            ColorNode.visit_and_set_color(current_vertex, color)
            for adjacent_index in current_vertex.edges:
                if adjacent_index not in discovered_set and \
                        self.find_node(adjacent_index).color == current_vertex.prev_color:
    ...
    def dfs(self, start_index, color):
        ...
        while len(index_stack) > 0:
            ...
            if current_vertex.visited is False:
                ...
                self.print_image()
                for adjacent_index in current_vertex.edges:
                    adjacent_vertex = self.find_node(adjacent_index)
                    if current_vertex.prev_color == adjacent_vertex.color:
                        index_stack.append(adjacent_index)
...
def main():
    ...
    debug = True
    if debug:
        in_data = open('../../../../../Downloads/tower.in')
    else:
        in_data = sys.stdin
